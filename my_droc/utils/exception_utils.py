...
def robot_error_handler(localss, locals, corr_rounds, li, step_name, code_as_policies):
    ...
    while True:
        correction = input(
            'I cannot reach that tatget. The solution is: 1. Modify your correction; 2. Delete that detection.')
        if '1' in correction:
            correction = input('Please tell me how to correct: ')
            add_to_log(correction, file_path=USER_LOG)
            try:
            ...
            tmp[li][step_name]['code when interrupted'] = code_as_policies.split(current_codeline)[1]
            tmp[li][step_name][
                f'Outcome {corr_rounds}'] = 'Wrong detection. Interrupted at codeline "' + current_codeline + '".'
...
def no_exception_handler(localss, locals, corr_rounds, li, step_name, failure_reasoning, use_interrupted_code,
                         code_as_policies):
    for k, v in localss.items():
        locals[corr_rounds][k] = v
    tmp = load_file(HISTORY_TMP_PATH)

...
from utils.perception.perception_utils import _parse_pos, _correct_past_detection, _get_task_pose, _get_task_detection, \
    _change_reference_frame, initialize_detection, set_policy_and_task, clear_saved_detected_obj
                ...
                for _, step_name in code_step.items():
                    ...
                    while True:
                        try:
                            ...
                            correction = input(
                                f'Please issue further corrections, or is the task "{parsed_step_name}" done: ')
                            if 'done' in correction.lower() or 'yes' in correction.lower() or correction.lower() == 'y':
                                break
                            else:
                                whole_prompt = prompt_correction_no_history + '\n' + '\n' + f"# Task: {correction}."
                                response = query_LLM(whole_prompt, ["# Task:", "# Outcome:"],
                                                     "cache/llm_voice_teleop.pkl")
                                _, code_as_policies = format_code(response)

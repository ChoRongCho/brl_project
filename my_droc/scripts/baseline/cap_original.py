...
from utils.string_utils import extract_array_from_str, replace_description_with_value, replace_code_with_no_description, \
    replace_strarray_with_str, \
    format_plan, format_code, get_lines_starting_with, format_dictionary, replace_brackets_in_file
from utils.perception.perception_utils import _parse_pos, _correct_past_detection, _get_task_pose, _get_task_detection, \
    _change_reference_frame, get_detected_feature, compare_feature, \
    get_initial_state, initialize_detection, get_considered_classes, get_objs, _update_object_state, \
    compare_text_image_sim, test_image, clear_saved_detected_obj, set_policy_and_task, set_saved_detected_obj
from utils.io.io_utils import read_py, HISTORY_TMP_PATH, add_to_log, load_file, USER_LOG, delete_file, \
    get_previous_tasks, save_information_perm, save_information, save_plan_info
...
from utils.exception_utils import InterruptedByHuman, GraspError, RobotError, PlanningError, WrongDetection, \
    interruption_handler, robot_error_handler, grasp_error_handler, detection_error_handler, no_exception_handler, \
...
def main():
    ...
    while True:
                ...
                for num, step_name in code_step.items():
                    ...
                    while True:
                        try:
                            add_to_log('-' * 80 + '\n' + code_as_policies + '\n' + '-' * 80, also_print=True)
                            exec(code_as_policies, globals(), localss)
                            _break, corr_rounds, code_as_policies, use_interrupted_code = no_exception_handler(localss,
                                                                                                               locals,
                                                                                                               corr_rounds,
                                                                                                               li,
                                                                                                               step_name,
                                                                                                               failure_reasoning,
                                                                                                               use_interrupted_code,
                                                                                                               code_as_policies)
                            if _break:
                                break
                            ...
                            code_as_policies = detection_error_handler(localss, locals, corr_rounds, li, step_name,
                                                                       code_as_policies)
                            ...
                            code_as_policies, corr_rounds = failure_reasoning(step_name, li, corr_rounds)
                    add_to_log(
                        f'********************Success! "{parsed_step_name}" is fulfilled !!!!!********************',
                        also_print=True)
        ...
        add_to_log("---------------------------Ready to move to next instruction...---------------------------",
                   also_print=True)
...
def retrieve_plan_info(li):
    ...
    if all_constraints_dict != {}:
        ...
        if has_image:
            ...
            print(
                f'Planning-level retrieval: Retrieving these constraints: {[all_constraints_list[i - 1] for i in constraint_index_list]}')
            return [all_constraints_list[i - 1] for i in constraint_index_list], [all_image_features_list[i - 1] for i
                                                                                  in constraint_index_list]
        else:
...
def replan(corr_rounds, li, step_name, original_plan, object_state, plan_features):
    ...
    task_constraint = get_lines_starting_with(response.text, 'Task constraint:', first=True).split('Task constraint: ')[
        1]
    robot_constraint = \
    get_lines_starting_with(response.text, 'Robot constraint:', first=True).split('Robot constraint: ')[1]
    updated_object_state = \
    get_lines_starting_with(response.text, 'Updated object state:', first=True).split('Updated object state: ')[1]
...
def retrieve_task_info(step_name):
    ...
    if previous_tasks == '':
        ...
        if response_dict["1"] == 'Yes':
            ...
            other_keys = [key for key in info_dict.keys() if
                          key != 'relative_pose' and key != 'image_feature' and key != 'code' and key != 'dino_image_feature']
...
def save_task_info(locals, step_name, corr_rounds, li):
    ...
    try:
        ...
        info_to_save = (
        {'relative_pose': relative_pose, 'image_feature': image_feature, 'dino_image_feature': dino_image_feature})
